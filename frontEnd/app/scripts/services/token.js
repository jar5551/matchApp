...
angular.module('frontEndApp')
    .factory('token', function ($rootScope, $http, $q, $cookies, $location, jwtHelper) {
        var user = {
                ...
                setUser: function (name, loggedIn) {
                    user.name = name;
                ...
                login: function (credentials) {
                    ...
                    $http.post($rootScope.apiRoot + 'users/login', credentials, config).then(function successCallback(response) {
                        ...
                        user.setLoginData(response.data.jwt, response.data.rto, response.data.user.name);
                ...
                setLoginData: function (jwt, rto, name) {
                    user.setToken(jwt);
                    user.setRefreshToken(rto);
                    user.setUser(name, true);
                },
                ...
                createAccount: function (data) {
                    ...
                    $http.post($rootScope.apiRoot + 'users/register', data, config).then(function successCallback(response) {
